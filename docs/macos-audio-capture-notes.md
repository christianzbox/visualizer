...
Spectra asks ScreenCaptureKit for shareable content directly instead of blocking on `CGPreflightScreenCaptureAccess()`. On macOS 14.4 or newer it prefers `SCShareableContent.currentProcess`, which can enumerate capture sources available to the current process without forcing a separate screen-recording preflight. If ScreenCaptureKit denies capture, Spectra shows a permission message and keeps Test Signal Mode available.
